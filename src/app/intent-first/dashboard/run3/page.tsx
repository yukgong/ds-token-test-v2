...
function Sidebar() {
  // Apply `dark` class so child neutral tokens resolve to dark palette
  ...
  // Active menu item — lighten overlay on dark background
  ...
  // Disabled menu — opacity only, no separate token
  ...
  // Logout — destructive/critical colour
...
function Header() {
  ...
  // Tooltip: dark solid background + contrasting text (inverse feel)
  // Use neutral.fill.high which is described as "배경과 완전히 반대되는 채움 색상"
  ...
  // Bell badge — problem/critical colour
  ...
  // Profile avatar — primary colour + inverse text
...
function ToastItem({
  ...
  return (
    <div
      ...
      style={{
        // NOTE: inline style strictly avoided — use token-driven classes below
      }}
    >
      <ToastPositiveContent bg={bg} border={border} iconColor={iconColor} textColor={textColor} icon={icon} message={message} />
    </div>
  );
}

// Separate per-intent toast components to avoid dynamic token strings
...
// Unused helper — remove to satisfy linter
function ToastPositiveContent(_props: {
...
function StatCardPositive({ label, value, change, icon }: { label: string; value: string; change: string; icon: React.ReactNode }) {
  const cardClass = css({
    backgroundColor: 'neutral.canvas.02',
    borderWidth: '1px',
    borderStyle: 'solid',
...
function StatCardCritical({ label, value, change, icon }: { label: string; value: string; change: string; icon: React.ReactNode }) {
  const cardClass = css({
    backgroundColor: 'neutral.canvas.02',
    borderWidth: '1px',
    borderStyle: 'solid',
...
function StatCardWarning({ label, value, change, icon }: { label: string; value: string; change: string; icon: React.ReactNode }) {
  const cardClass = css({
    backgroundColor: 'neutral.canvas.02',
    borderWidth: '1px',
    borderStyle: 'solid',
...
function ResourceCardPositive({ label, percent, statusText }: { label: string; percent: number; statusText: string }) {
  const cardClass = css({
    backgroundColor: 'neutral.canvas.02',
    borderWidth: '1px',
    borderStyle: 'solid',
...
function ResourceCardWarning({ label, percent, statusText }: { label: string; percent: number; statusText: string }) {
  const cardClass = css({
    backgroundColor: 'neutral.canvas.02',
    borderWidth: '1px',
    borderStyle: 'solid',
...
function ResourceCardCritical({ label, percent, statusText }: { label: string; percent: number; statusText: string }) {
  const cardClass = css({
    backgroundColor: 'neutral.canvas.02',
    borderWidth: '1px',
    borderStyle: 'solid',
...
function EventTable() {
  ...
  // Row avatar: neutral background + text
  const avatarClass = css({
    width: '32px',
    height: '32px',
    borderRadius: '9999px',
  ...
  // Status cells — per intent
  const statusPositiveClass = css({ display: 'flex', alignItems: 'center', gap: '6px' });
  ...
  const statusCriticalClass = css({ display: 'flex', alignItems: 'center', gap: '6px' });
  ...
  const statusWarningClass = css({ display: 'flex', alignItems: 'center', gap: '6px' });
  ...
  const rows = [
    { initial: '홍', name: '홍길동', event: '로그인 성공', time: '14:32', status: '정상 처리됨', statusVariant: 'positive' as const },
    { initial: '김', name: '김철수', event: '결제 실패', time: '14:28', status: '문제 발생', statusVariant: 'critical' as const },
    { initial: '이', name: '이영희', event: '비밀번호 5회 오류', time: '14:21', status: '주의 필요', statusVariant: 'warning' as const },
    { initial: '박', name: '박민준', event: '회원가입', time: '14:15', status: '정상 처리됨', statusVariant: 'positive' as const },
    { initial: '시', name: '시스템', event: 'API 응답 지연', time: '14:09', status: '주의 필요', statusVariant: 'warning' as const },
  ];
          ...
          {rows.map((row, i) => {
            ...
            return (
              <tr key={i}>
                ...
                <td className={cellClass}>
                  {row.statusVariant === 'positive' && (
                    <div className={statusPositiveClass}>
                  ...
                  {row.statusVariant === 'critical' && (
                    <div className={statusCriticalClass}>
                  ...
                  {row.statusVariant === 'warning' && (
                    <div className={statusWarningClass}>
...
function TogglePanel() {
  ...
  const titleClass = css({ color: 'neutral.text.base', fontSize: '16px', fontWeight: '700', marginBottom: '4px' });
  ...
  // Disabled row — opacity 0.38 on the whole row
  ...
  // Toggle ON: primary fill background + white handle
  ...
  // Toggle OFF: neutral fill background
  ...
  // White circular handle (neutral.canvas.01 ≈ white on light theme)
