...
> **실험의 변천**: 이 프로젝트는 처음부터 **토큰을 직접 골라 쓰는 주체(AI 코드 생성 포함)가 명칭 체계에 따라 토큰을 얼마나 정확히 고르는가**를 보는 실험이다. 초기엔 *role-first(기존) vs property-first(신규)* 두 명칭 체계를 비교했으나, ① 기존 role-first가 오용률 8.17%로 조기 탈락·아카이브되고, ② 신규 후보를 intent-first(IF)로 두고 **설명·순서 등 변수를 격리**하는 다중 arm 비교로 발전했다. "명칭 체계가 토큰 사용 정확도에 직결된다"는 큰 줄기는 그대로다.
...
panda.config.intent-first.ts             →  styled-system-intent-first/            (intent-first, IF+/IF−/IFC 공유)
panda.config.intent-first-stepnames.ts   →  styled-system-intent-first-stepnames/  (intent-first, step 어휘 변경, IFS)
panda.config.intent-first-slotfirst.ts   →  styled-system-intent-first-slotfirst/  (slot-first 명칭, INV)
```

> IFC arm은 IF−/IF+와 토큰·값이 동일하므로 별도 config 없이 `styled-system-intent-first`를 공유한다(프롬프트 토큰 목록에 canvas/surface 구분 힌트만 추가한 변형, per-token 설명은 없음).
...
| arm | 네이밍 | 설명 | 비고 |
|---|---|---|---|
| **SF** | slot-first | ✗ (자동완성 동등) | 외부 `property-first` 토큰 체계 (slot-first 계열·비교 대조군) |
| **IF−** | intent-first | ✗ | IF+와 설명 유무만 다름 |
| **IFC** | intent-first | ✗ | IF−(이름만)에 **canvas/surface 구분 힌트 한 단락만** 추가 (per-token 설명 없음) |
| **IF+** | intent-first | ✓ | 신규 후보 (mildang 자체 설계) |
| **IFS** | intent-first (step 어휘 변경) | ✓ | 셀 IF와 **동일 토큰·색상값·순서**, step 단어만 `low/base/high/highest/lowest` → `subtle/default/strongest/emphasis/muted` |
| **INV** | slot-first | ✓ | 셀 IF와 **동일 토큰·색상값**을 순서만 반전(`{intent}.{slot}` ↔ `{slot}.{intent}`) |

- **IF+ ↔ IF−** 는 토큰·순서·styled-system이 모두 동일하고 **설명 유무만** 다름 → 설명 효과 격리.
- **IF+ ↔ INV** 는 동일 토큰·색상값을 **순서만** 반전, 설명은 둘 다 있음 → 순서 효과 격리.
- **IF+ ↔ IFS** 는 동일 토큰·색상값·순서에서 **step 단어만** 교체, 설명도 함께 변경 → step 어휘 효과 격리.
- **IF− ↔ IFC** 는 이름만인 IF−에 **canvas/surface 구분 한 단락만** 추가 → 단일 핀포인트 힌트 효과 격리.

> **용어:** SF·INV는 둘 다 슬롯/속성이 먼저 오는 **slot-first 계열**이다(`{slot}.{intent}...`). 차이는 — SF는 외부에서 설계된 별도 `property-first` 토큰 체계(설명 없음, `bg`/`surface`/`icon` 등이 첫 세그먼트), INV는 셀 IF(intent-first)를 순서만 반전한 것(설명 있음, 셀 IF와 동일 값).

### 페이지

```
/property-first/{dashboard,billing,notifications,settings}/run{1,2,3}   SF 생성물
/e-nodesc/{dashboard,billing,notifications,settings}/run{1,2,3}         IF− 생성물
/e-canvas-hint/{dashboard,billing,notifications,settings}/run{1,2,3}    IFC 생성물
/intent-first/{dashboard,billing,notifications,settings}/run{1,2,3}     IF+ 생성물
/step-rename/{dashboard,billing,notifications,settings}/run{1,2,3}      IFS 생성물
...
|---|---|---|---|---|---|
| **INV** | slot-first (셀 IF 토큰) | ✓ | **0.34%** | 7 / 2082 | 세로 구분선·토글 knob (소수) |
| **IF+** | intent-first | ✓ | **0.37%** | 7 / 1910 | 세로 구분선·토글 knob (소수) |
| **IFS** | intent-first (step 어휘) | ✓ | **0.47%** | 10 / 2131 | 세로 구분선·토글 knob (소수) + intent 1 |
| **IFC** | intent-first + canvas/surface 힌트만 | ✗ | **0.75%** | 16 / 2129 | 토글 knob·세로 구분선 + `primary/critical.canvas` 환각 6 |
| **SF** | slot-first (property-first 체계) | ✗ | **0.85%** | 17 / 2006 | slot 오용 + 아이콘↔텍스트 혼용 |
| **IF−** | intent-first | ✗ | **1.70%** | 39 / 2300 | surface를 레이아웃 배경에 사용 (canvas 오답) |

정규화 오용률 = 오용 총건수 ÷ 시맨틱 컬러 토큰 사용 총횟수(생성 verbosity에 의한 분모 오염 방지). IFC만 부분 힌트 부작용으로 `primary/critical.canvas` 환각 6건(아래 4번), 나머지 5개 arm은 환각 0.

### 변수 격리 결과

1. **설명 효과 (IF− → IF+, 약 4.6× 감소)** — 결정적. IF−의 오용은 거의 전부 `neutral.surface.*`를 카드·컨테이너·테이블 같은 **레이아웃 배경에 사용**(정답 `neutral.canvas.*`). 설명이 없으면 모델이 **canvas(레이아웃 레이어) vs surface(컴포넌트)** 를 구분하지 못함. → **설명이 오용률의 지배 변수.**
...
본 실험은 **IF 계열(intent-first) 토큰의 설계자가 평가자 본인**이라는 비대칭 위에서 수행되었다. SF는 외부 설계 토큰이라 동일 조건을 부여하지 못했고, 결과 수치(특히 SF 0.85%)는 이 한계를 감안해 해석해야 한다.

1. **SF에 "설명 추가본"을 실험하지 못함** — IF 계열은 설명 유무(IF−/IF+)를 모두 측정했지만 SF는 설명 없음 단일 조건만 측정. SF 토큰의 **설계 의도를 평가자가 알 수 없어**(외부 산출물) 부합하는 설명을 작성할 수 없었기 때문(추정 설명은 그 자체로 새 편향). 따라서 "설명이 지배 변수"라는 결론은 **IF 계열 내부에서만 인과적으로 입증**된 것.
2. **SF 채점의 정답 판정 한계 → 실측 오용률 과소평가 가능** — 뷰어 렌더 확인 결과 **어두운 배경 + 대비 텍스트를 지시한 사이드바가 제대로 구현되지 않은** 케이스가 관찰됨. 그러나 현재 채점 프롬프트는 이를 오용으로 잡지 못함(SF의 정답 토큰 조합을 확정 불가 → IF 계열에 대해서만 신뢰성 있게 채점). **SF 실제 오용률은 집계된 0.85%보다 높을 수 있음.**
3. **SF 토큰 구조 결함 (inverse 커버리지)** — SF는 다크/대비 영역을 `inverse` 토큰으로 처리하나, **모든 role × 슬롯 조합마다 inverse를 빠짐없이 정의하는 것은 비현실적**. 누락 조합에서 §2의 구현 실패가 발생. → inverse를 슬롯별로 채우기보다 **국소 부위(예: 사이드바)를 통째로 다크 테마로 전환**하는 방식이 더 견고할 것.
...
5. **`brand` ≠ `primary` (밀당 색상 정체성)** — 밀당은 brand가 초록인데 **진행률·완료 등 상태 표현도 대부분 초록**이고 표시할 상태가 많아 화면이 온통 초록·알록달록. 그래서 **메인 컬러를 중립 검정**으로 잡아 `primary ≠ brand`. IF 계열엔 `brand`를 두지 않고 **`primary`만** 둠(필요 시 `secondary` 추가 가능). brand/primary를 동일시하는 토큰 체계와는 전제가 다르므로 비교 시 유의.
...
#   SF  → eval/inputs/tokens-property-first-nodesc.md     (이름만)
#   IF−  → eval/inputs/tokens-intent-first-nodesc.md         (이름만)
#   IFC  → eval/inputs/tokens-intent-first-canvas-hint.md     (이름만 + canvas/surface 구분 힌트만, per-token 설명 없음)
#   IF+  → eval/inputs/tokens-intent-first-desc.md            (이름+설명)
#   IFS  → eval/inputs/tokens-intent-first-stepnames-desc.md  (이름+설명, step 어휘 변경)
...
panda.config.property-first.ts           SF config
panda.config.intent-first.ts             IF+/IF− config (intent-first, 공유)
panda.config.intent-first-stepnames.ts   IFS config (셀 IF 값을 step 어휘 변경 이름으로)
panda.config.intent-first-slotfirst.ts   INV config (셀 IF 값을 slot-first 이름으로)
src/
  tokens/
    colors/               primitive 색상 (gray, red, green, blue, orange, neutral, mildangPT 등)
    semanticColors/
      property-first/             SF 토큰 (mildang.ts)
      intent-first/               IF 토큰 (mildang.ts; slotfirst는 자동 변환)
      intent-first-stepnames/     IFS 토큰 (mildang.ts; 셀 IF의 step 어휘 변경본)
  app/
    property-first/{page}/run{n}/   SF 생성물
    e-nodesc/{page}/run{n}/         IF− 생성물
    e-canvas-hint/{page}/run{n}/    IFC 생성물 (styled-system-intent-first 공유)
    intent-first/{page}/run{n}/     IF+ 생성물
    step-rename/{page}/run{n}/      IFS 생성물
