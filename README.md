...
panda.config.intent-first.ts             →  styled-system-intent-first/            (intent-first, E+/E− 공유)
...
토큰의 **네이밍 순서**(intent-first / slot-first / property-first) · **설명 유무** · **step 어휘**를 독립 변수로 분리해 비교한다.
...
5 arm × 4 page × 3 run = **60셀**. 각 arm은 동일한 중립 스펙으로 AI가 **독립 생성**한다(같은 UI를 렌더하는 게 아님).
...
| **시각 검증** | 60셀 렌더 → 스크린샷 → 뷰어 | 스펙대로 구현됐는가 (예: 다크 사이드바) |
...
전 arm 환각(목록에 없는 토큰 생성) **0건**. 정규화 오용률 = 오용 총건수 ÷ 시맨틱 컬러 토큰 사용 총횟수(생성 verbosity에 의한 분모 오염 방지).
...
4. **설계 함의** — 설명 없는 둘 중에서는 오히려 **E−(1.70%) > PF(0.85%)**. property-first는 canvas/surface 구분이 없어(레이아웃·컴포넌트 모두 `surface.neutral.*`) **그 실수를 할 수조차 없으므로** 표현력이 낮아 역설적으로 안전.

> 결론: 토큰 체계의 표현력(canvas 레이어 등)은 **설명과 짝을 이룰 때만** 가치를 가진다. 오용률의 핵심 변수는 **설명**이며, 네이밍 표면(순서 intent-first/slot-first · step 어휘 base/default 등)은 거의 무관하다.
...
# 채점·집계·스샷
node eval/_rebuild.mjs   # 5-arm 집계 → summary.{json,md}
node eval/_shot.mjs      # 60셀 렌더 → eval/viewer/shots/ → 뷰어 자동 반영
```
...
eval/
  inputs/    중립 스펙(spec-{billing,dashboard,notifications,settings}) + 토큰목록 5종
  results/   런별 채점 JSON(60) + summary.{json,md}
  viewer/    index.html + data.js + shots/ (5-arm 뷰어)
  SCORING.md 채점 룰북
```
