# 셀 E — 사용 가능한 시맨틱 컬러 토큰 (intent-first, 이름만 + canvas/surface 구분 설명)
...
## **[배경(background) 슬롯 구분 — canvas vs surface vs fill]**
배경 계열은 셋이 역할이 다르므로 엄격히 구분한다.
- **`canvas.*` = 레이아웃 배경 레이어.** 페이지 전체 배경, 그리고 카드·패널·컨테이너·박스·테이블 등 **'구조를 이루는 면'의 배경**. 깊이 순서: `canvas.01`(페이지 최하단 바탕) → `canvas.02`(그 위에 놓이는 카드/패널/컨테이너) → `canvas.03`(컨테이너 위에 한 단계 더 얹히는 면).
- **`surface.*` = 개별 컴포넌트 자체의 표면색.** 입력창·버튼·뱃지 등 **컴포넌트의 배경**(레이아웃 컨테이너의 배경이 아님).
- **`fill.*` = 토글·프로그레스 등의 트랙/채움색.**
- → **핵심: 카드·패널·컨테이너·페이지 등 레이아웃/구조 배경에는 `surface`가 아니라 `canvas`를 쓴다.** `surface`는 컴포넌트 표면 전용.
